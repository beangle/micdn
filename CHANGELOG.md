...
- **镜像**：新增 scratch 静态镜像 `Dockerfile.scratch` + `scripts/build_scratch.sh`（LDC musl 全静态、无 shell / apk / 调试工具；仅携带 curl 及其动态依赖与 CA 证书）
...
- 文档：README「运行时系统命令依赖」收敛为仅 `curl` 一个；`docs/build_linux.md` 补充交付前 `ldd` / RPATH / `libgcc_s` 依赖体检
