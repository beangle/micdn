...
- 压缩由独立后台线程完成（写 `tmp` 后原子 `rename`），不阻塞请求线程；仅当压缩后确实更小才落盘，超过 8MB 的文件不压缩。
...
- 超过 8MB 的文件不压缩（`src/micdn/gzip.d` 中 `maxGzipFileSize`）。这是一个保护性上限，而非性能优化目标：
...
- 典型前端静态资源（JS/CSS/HTML 等）远小于 8MB，该上限不影响正常部署；如需调整，直接修改 `maxGzipFileSize` 即可（当前为硬编码，未做配置化）。
